...
export default function Order({order} :any) {
    ...
    React.useEffect(() => {
        // const order = async ()=>{
        //     let res = await fetch("http://localhost:3000/api/orderById");
...
export const getServerSideProps = async(ctx:any) => {
        ...
        let res = await fetch(`${dev ? DEV_URL : PROD_URL}/api/orderById?id=${orderId}`);
        // await fetch(`/api/orderById`)
        // let order = await db.collection("Orders").find({orderId: orderId});
        // console.log(order)
    return{
        props:{
            // order : orderId
